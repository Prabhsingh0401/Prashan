...
import { Bell, User, Settings, FileText } from "lucide-react";
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 flex items-start justify-end p-4 md:p-6 pointer-events-none">
      ...
      <div className="flex items-center gap-2 pointer-events-auto bg-white/30 dark:bg-white/5 backdrop-blur-3xl p-1.5 rounded-[1.25rem] border border-white/40 dark:border-white/10 shadow-xl shadow-black/5 dark:shadow-black/40">
        ...
        {/* Improved User Profile Icon */}
        <button className="flex items-center justify-center w-10 h-10 ml-1 rounded-full overflow-hidden bg-black/5 dark:bg-white/10 backdrop-blur-md border border-black/10 dark:border-white/10 text-foreground shadow-sm ring-2 ring-transparent transition-all hover:ring-foreground/20 hover:scale-105 active:scale-95 shrink-0">
          {photoUrl ? (
            <img 
              src={photoUrl} 
              alt="User" 
